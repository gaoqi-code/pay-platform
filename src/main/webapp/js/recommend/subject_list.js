...
function infoDelete(id){
	$.messager.confirm(titleInfo, '确定删除该条信息？', function(r){
		if(r){
			var rowInfo =  $('#detail_table').datagrid('getSelected');
			if(rowInfo){
				$.post("subject/delete.json",{subjectId:rowInfo.subjectId,subjectName:rowInfo.subjectName},function(data){
					if(1==data.code){
...
function submitModel(){
	...
	if(null==pkId||pkId==""){
		$.post("subject/add.json",dataInfo,function(data){
			$('#detail_table').datagrid('reload');
			$('#detail_dialog').dialog('close');
			$.messager.show({title:titleInfo,msg:'保存成功！',timeout:timeoutValue,showType:'slide'});
		},"json");
	}else{
		$.post("subject/update.json",dataInfo,function(data){
			$('#detail_table').datagrid('reload');
			$('#detail_dialog').dialog('close');
...
function initDataGrid(){
	$('#detail_table').datagrid({
		...
		columns:[[
			...
			{field:'subjectPic',title:'缩略图',
				formatter:function(value){
					return '<img alt="" src="'+value+'" style="width:200px;height:100px;">';
				}
			},
			{field:'subjectBgImg',title:'背景图',
				formatter:function(value){
					return '<img alt="" src="'+value+'" style="width:200px;height:100px;">';
				}
			},
			{field:'isEffective',title:'状态',width:40,
				formatter:function(value){
					if(1==value){
						return '有效';
					}else{
			...
			{field:'subjectId',title:'操作',width:60,
				formatter:function(value){
